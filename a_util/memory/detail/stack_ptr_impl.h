...
{
    new (address()) T();
    setFlag(InitializeStatus, IsConstructed);
}
...
template <typename T, std::size_t StackSize, std::size_t Alignment>
inline StackPtr<T, StackSize, Alignment>& StackPtr<T, StackSize, Alignment>::
    operator=(StackPtr other)
{
    other.swap(*this);
    return *this;
}
...
inline void StackPtr<T, StackSize, Alignment>::reset()
{
    if (isConstructed())
    {
        ...
        setFlag(InitializeStatus, IsDestroyed);
    }
}
...
inline void StackPtr<T, StackSize, Alignment>::swap(StackPtr& other)
{
    ...
    {
        std::size_t remaining_bytes_to_swap = StorageSize;
    }
    ...
    { // don't use memory::zero to avoid link dependency to memory component
        ...
        std::fill_n(reset_after_swap->_storage, StackSize, 0);
    }
}
...
inline void StackPtr<T, StackSize, Alignment>::setFlag(StackPtrFlagPositions position,
                                                       StackPtrFlags flag)
{
    _storage[position] = flag;
}

template <typename T, std::size_t StackSize, std::size_t Alignment>
inline bool StackPtr<T, StackSize, Alignment>::isConstructed() const
{
    return _storage[InitializeStatus] == IsConstructed;
}
                       ...
                       const StackPtr<T, StackSize, Alignment>& rhs)
{
    ...
    else if (!(*lhs == *rhs)) // fully intended, requires only comparison operator!
    {
        return false;
    }
