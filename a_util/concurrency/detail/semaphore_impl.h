...
{
    a_util::concurrency::unique_lock<Mutex> lock(_mutex);
    _lock_count++;
    _cv.notify_all();
}
...
inline bool
    basic_semaphore<Mutex, CondVar>::wait_for(const a_util::chrono::duration<Rep, Period>& timeout)
{
    if (timeout.count() < 0)
    {
    ...
    const timestamp_t until =
        ...
        a_util::chrono::duration_cast<a_util::chrono::microseconds>(timeout).count();
    ...
    while (_lock_count == 0 && !is_timeout)
    {
        ...
        is_timeout =
            (wait_time <= 0) || (_cv.wait_for(lock, a_util::chrono::microseconds(wait_time)) ==
