...
struct Timer::Implementation
{
    ...
    mutable a_util::concurrency::recursive_mutex mutex_timer;
    a_util::concurrency::recursive_mutex mutex_callback;
    ...
    static void CALLBACK TimeProc(UINT uTimerID, UINT, DWORD_PTR dwUser, DWORD_PTR, DWORD_PTR)
    {
        ...
        {
            concurrency::unique_lock<concurrency::recursive_mutex> lock(_impl->mutex_callback);
            _impl->callback();
        }
...
#else
    a_util::memory::unique_ptr<a_util::concurrency::thread> thread;
    ...
    void ThreadFunc()
    {
        ...
        {
            {
                concurrency::unique_lock<concurrency::recursive_mutex> lock(mutex_callback);

#ifdef __QNX__
                struct _pulse pulse;
                while (!stop_flag &&
                       MsgReceivePulse(channel_id, &pulse, sizeof(pulse), nullptr) < 0)
#else
...
{
    concurrency::unique_lock<concurrency::recursive_mutex> lock(_impl->mutex_timer);
...
#endif
        _impl->thread.reset(new concurrency::thread(&Implementation::ThreadFunc, _impl));
    }
#endif
...
bool Timer::stop()
{
    ...
    _impl->is_running = false;
    lock.unlock();
    concurrency::unique_lock<concurrency::recursive_mutex> lock2(_impl->mutex_callback);
#else
    ...
    if (_impl->thread->get_id() != a_util::concurrency::this_thread::get_id())
    {
