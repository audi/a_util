...
/**
 * @brief A smart pointer allocating its memory only on the stack
...
public:
    ...
    /**
     * @brief Initializes the storage by calling the copy constructor of type @c T
    ...
    /**
     * @brief Operator overload to use this class like a raw pointer
    ...
    /**
     * @brief Operator overload to use this class like a raw pointer - for const correctness
     * @returns Pointer to the storaged object of type @c T
     * @warning No check is performed whether the object was really constructed
     */
    ...
    /**
     * @brief Resets storage with @c data and, if constructed, destructs previously storaged object
     * @param[in] data Object of type @c T whose content gets copy constructed into the storage
     */
    void reset(const T& data);

    /**
     * @brief Swap storage of *this with storage of @c other stack pointer
...
private:
    /**
     * @brief Get pointer to beginning of the storage
     * @return Pointer to the beginning of the storage
     */
    void* address();

    /**
     * @brief Get pointer to beginning of the storage provided for const correctness
     * @return Pointer to the beginning of the storage
     */
    const void* address() const;

    /**
     * @brief Sets a @c flag to the appropriate @c position in the overhead section of the storage
    ...
    /**
     * @brief Check whether the object hidden by the storage got constructed
     ...
     * @tparam[in] U Managed type
     * @tparam[in] SSize Size the managed type occupies
     * @tparam[in] A Byte alignment of the storage
...
/**
 * @brief Swap storages of two objects of type @c StackPtr
 ...
 * @tparam[in] T Managed type
 * @tparam[in] StackSize Size the managed type occupies
 * @tparam[in] Alignment Byte alignment of the storage
 * @param[in] lhs Left hand side object
 * @param[in] rhs Right hand side object
