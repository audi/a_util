...
#endif
#include "a_util/strings/impl/std_to_detail.h"
#include "a_util/strings/impl/strings_format_detail.h"
...
bool isFloatingPoint(const char* first, std::size_t str_length)
{
    ...
    if (!contains_floating_point && (first != fractional)) // dot or comma must not be first char
    {
...
{ // integral is also a floating point; floating points can be converted to bool
    ...
    if (is_convertible)
    {
        ...
        if (std::string::npos == dot_index) // no dot
        {
