...
static int32_t GetFSAttributes(const Path& path)
{
...
{
#ifdef WIN32
    FILE* file = _wfopen(strings::unicode::utf8ToUtf16(file_path.toString()).c_str(), L"rb");
#else
...
{
#ifdef WIN32
    FILE* file = _wfopen(strings::unicode::utf8ToUtf16(file_path.toString()).c_str(), L"wb");
#else
        ...
        for (int idx = 0; idx < count; ++idx)
        {
            ...
            if (fnmatch("*", char_, 0) == 0)
            {
                ...
                if ((flags & ED_FILES) != 0)
                {
                    ...
                    if (0 == stat64(tmp_dir.toString().c_str(), &buffer))
                    {
                        if (S_ISREG(buffer.st_mode)) // is a normal file
                        {
                            vec_entries.push_back(tmp_dir);
                        }
                    }
