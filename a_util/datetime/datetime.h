...
/// Date class - represents a date consisting of year, month and day
/// @warning: This class does not enforce date consistency! "42.102.20000" is a valid date!
class Date
{
public:
     ...
     * @retval False of if the date string is invalid, true otherwise
     */
     ...
     * supported
     * @param format_str The format of the string representation (see @ref cDateTime::Format, can be
     * empty).
     * @return A string representation of the stored date.
     * @note Throws std::invalid_argument if the format string is invalid
     */
...
/// Time class - represents a time of day (00:00:00,0 - 23:59:59,999999) to the microsecond
...
public:
     ...
     * @retval False if the time string format is invalid, true otherwise
     */
 ...
 * @brief The reference point in time.
 *
...
{
    /**
     * @brief Reference point in time is noon on Monday, January 1, 4713 BC
     * (<a href="https://en.wikipedia.org/wiki/Julian_day">Julian day</a>)
     */
    MicroSecondsSinceJulianDateOrigin = 0,
    /**
     * @brief Reference point in time is midnight on Thursday, 1 January 1970
...
public:
    ...
    /**
     *
     * Format the date/time string.  Returns a formated string containing date/time information.
     * The format argument consists of one or more codes; as in printf, the formatting codes are
     * preceded by a percent sign (\c '\%'). Characters that do not begin with \c \% are copied
     * unchanged.
     *
     ...
     * @retval True if the conversion is OK, in this case the local data structures ares updated
     * with the new values
     * @remarks the counterpart is @ref cDateTime::Format
     */
