                 ...
                 std::string& std_err)
{
    ...
    HANDLE childStd_ERR_Rd = NULL;
    HANDLE childStd_ERR_Wr = NULL;
    CreatePipe(&childStd_ERR_Rd, &childStd_ERR_Wr, &sec_attributes, 0);

    // connect pipe to file stream - the most tricky part here
    FILE* fd_pipe = _fdopen(_open_osfhandle((long) childStd_OUT_Rd, 0), "r");
    FILE* fd_pipe_err = _fdopen(_open_osfhandle((long) childStd_ERR_Rd, 0), "r");
