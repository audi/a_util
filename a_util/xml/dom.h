...
/// own data type for DOMElement lists
typedef std::list<DOMElement> DOMElementList;
/// own data type for DOM Attributes
...
public:
     ...
     * @retval @c false if its an invalid attribute name, @c true otherwise
     */
     ...
     * See @ref DOMElement::findNode() for the syntax.
     *
     * @param[in] query The query string
 ...
 * @param lhs [in] Left instance
 * @param rhs [in] Right instance
 * @return @c true if the two instances are not equal (deep comparison)
 */
bool operator!=(const DOMElement& lhs, const DOMElement& rhs);

/// DOM implements a XML parser, able to read and write Standard XML files
class DOM
{
public:
    /// CTOR
